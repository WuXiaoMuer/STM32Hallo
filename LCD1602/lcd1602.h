...
uint8_t ReadData()
{
	uint8_t dat=0;
	//DataDir('I');
	if(HAL_GPIO_ReadPin(GPIOB, D0_Pin)==GPIO_PIN_SET) dat|=0x01;
	if(HAL_GPIO_ReadPin(GPIOB, D1_Pin)==GPIO_PIN_SET) dat|=0x02;
	if(HAL_GPIO_ReadPin(GPIOB, D2_Pin)==GPIO_PIN_SET) dat|=0x04;
	if(HAL_GPIO_ReadPin(GPIOB, D3_Pin)==GPIO_PIN_SET) dat|=0x08;
	if(HAL_GPIO_ReadPin(GPIOB, D4_Pin)==GPIO_PIN_SET) dat|=0x10;
	if(HAL_GPIO_ReadPin(GPIOB, D5_Pin)==GPIO_PIN_SET) dat|=0x20;
	if(HAL_GPIO_ReadPin(GPIOB, D6_Pin)==GPIO_PIN_SET) dat|=0x40;
	if(HAL_GPIO_ReadPin(GPIOB, D7_Pin)==GPIO_PIN_SET) dat|=0x80;
	return dat;
}
...
void WriteData(uint8_t dat)
{
	...
	HAL_GPIO_WritePin(GPIOB, Set_Pins, GPIO_PIN_SET);
	HAL_GPIO_WritePin(GPIOB, Rst_Pins, GPIO_PIN_RESET);
}
