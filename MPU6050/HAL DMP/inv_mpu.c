...
int mpu_init(void)
{
    ...
    printf("\r[mpu_init] Starting MPU6050 initialization...\r\n");
    
    /* Reset device. */
    printf("[mpu_init] Resetting MPU6050 device...\r\n");
    data[0] = BIT_RESET;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data)) {
        printf("[mpu_init] Reset failed\r\n");
        return -1;
    }
    delay_ms(100);
    printf("[mpu_init] Reset successful\r\n");

    /* Wake up chip. */
    printf("[mpu_init] Waking up MPU6050 chip...\r\n");
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, data)) {
        printf("[mpu_init] Wake up failed\r\n");
        return -1;
    }
    printf("[mpu_init] Wake up successful\r\n");

#if defined MPU6050
    /* Check product revision. */
    printf("[mpu_init] Checking product version...\r\n");
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, data)) {
        printf("[mpu_init] Product version read failed\r\n");
        return -1;
    }
    ...
    if (rev) {
        ...
        if (rev == 1 || rev == 7 || rev == 6)//add7
            st.chip_cfg.accel_half = 1;
        else if (rev == 2)
            st.chip_cfg.accel_half = 0;
        else {
            printf("[mpu_init] Unsupported product version: %d\r\n", rev);
            return -1;
        }
    } else {
        if (i2c_read(st.hw->addr, st.reg->prod_id, 1, data)) {
            printf("[mpu_init] Product ID read failed\r\n");
            return -1;
        }
        rev = data[0] & 0x0F;
        if (!rev) {
            printf("[mpu_init] Product ID is 0, device incompatible or MPU3050\r\n");
            return -1;
        } else if (rev == 4) {
            printf("[mpu_init] Half sensitivity version found\r\n");
            st.chip_cfg.accel_half = 1;
        } else
            st.chip_cfg.accel_half = 0;
    }
    printf("[mpu_init] Product version check passed, version: %d\r\n", rev);
#elif defined MPU6500
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    printf("[mpu_init] Setting gyroscope full scale range(2000dps)...\r\n");
    if (mpu_set_gyro_fsr(2000)) {
        printf("[mpu_init] Gyroscope full scale setup failed\r\n");
        return -1;
    }
    
    printf("[mpu_init] Setting accelerometer full scale range(2g)...\r\n");
    if (mpu_set_accel_fsr(2)) {
        printf("[mpu_init] Accelerometer full scale setup failed\r\n");
        return -1;
    }
    
    printf("[mpu_init] Setting low pass filter(42Hz)...\r\n");
    if (mpu_set_lpf(42)) {
        printf("[mpu_init] Low pass filter setup failed\r\n");
        return -1;
    }
    
    printf("[mpu_init] Setting sample rate(50Hz)...\r\n");
    if (mpu_set_sample_rate(50)) {
        printf("[mpu_init] Sample rate setup failed\r\n");
        return -1;
    }
    
    printf("[mpu_init] Configuring FIFO...\r\n");
    if (mpu_configure_fifo(0)) {
        printf("[mpu_init] FIFO configuration failed\r\n");
        return -1;
    }
...
#ifdef AK89xx_SECONDARY
    printf("[mpu_init] Setting clock source...\r\n");
    setup_compass();
    if (mpu_set_compass_sample_rate(10)) {
        printf("[mpu_init] Clock source setup failed\r\n");
        return -1;
    }
#else
    printf("[mpu_init] Setting bypass mode...\r\n");
    /* Already disabled by setup_compass. */
    if (mpu_set_bypass(0)) {
        printf("[mpu_init] Bypass mode setup failed\r\n");
        return -1;
    }
#endif

    printf("[mpu_init] Disabling all sensors...\r\n");
    mpu_set_sensors(0);
    
    printf("[mpu_init] MPU6050 basic initialization completed!\r\n");
    return 0;
}
...
uint8_t mpu_dmp_init(void)
{
    ...
    MPU_IIC_Init(); 	//?????IIC????
    printf("[mpu_dmp_init] I2C initialization completed\r\n");
    
    printf("[mpu_dmp_init] Starting MPU6050 initialization...\r\n");
    if(mpu_init()==0)	//?????MPU6050
    {
        printf("[mpu_dmp_init] MPU6050 basic initialization successful\r\n");
        
        printf("[mpu_dmp_init] Setting sensors (gyroscope + accelerometer)...\r\n");
        res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//Set required sensors
        if(res) {
            printf("[mpu_dmp_init] Sensor setup failed, error code: %d\r\n", res);
            return 1;
        }
        printf("[mpu_dmp_init] Sensor setup successful\r\n");
        
        printf("[mpu_dmp_init] Configuring FIFO...\r\n");
        res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL);//Set FIFO
        if(res) {
            printf("[mpu_dmp_init] FIFO configuration failed, error code: %d\r\n", res);
            return 2;
        }
        printf("[mpu_dmp_init] FIFO configuration successful\r\n");
        
        printf("[mpu_dmp_init] Setting sample rate (%dHz)...\r\n", DEFAULT_MPU_HZ);
        res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//Set sample rate
        if(res) {
            printf("[mpu_dmp_init] Sample rate setup failed, error code: %d\r\n", res);
            return 3;
        }
        printf("[mpu_dmp_init] Sample rate setup successful\r\n");
        
        printf("[mpu_dmp_init] Loading DMP firmware...\r\n");
        res=dmp_load_motion_driver_firmware();		//Load DMP firmware
        if(res) {
            printf("[mpu_dmp_init] DMP firmware loading failed, error code: %d\r\n", res);
            return 4;
        }
        printf("[mpu_dmp_init] DMP firmware loaded successfully\r\n");
        
        printf("[mpu_dmp_init] Setting orientation matrix...\r\n");
        res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//Set gyroscope orientation
        if(res) {
            printf("[mpu_dmp_init] Orientation matrix setup failed, error code: %d\r\n", res);
            return 5;
        }
        printf("[mpu_dmp_init] Orientation matrix setup successful\r\n");
        
        printf("[mpu_dmp_init] Enabling DMP features...\r\n");
        res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//Set DMP features
                               DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
                               DMP_FEATURE_GYRO_CAL);
        if(res) {
            printf("[mpu_dmp_init] DMP feature enable failed, error code: %d\r\n", res);
            return 6;
        }
        printf("[mpu_dmp_init] DMP features enabled successfully\r\n");
        
        printf("[mpu_dmp_init] Setting DMP FIFO rate...\r\n");
        res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//Set DMP output rate (max 200Hz)
        if(res) {
            printf("[mpu_dmp_init] DMP FIFO rate setup failed, error code: %d\r\n", res);
            return 7;
        }
        printf("[mpu_dmp_init] DMP FIFO rate setup successful\r\n");
        
        printf("[mpu_dmp_init] Running self-test...\r\n");
        res=run_self_test();		//Self-test
        if(res) {
            printf("[mpu_dmp_init] Self-test failed, error code: %d\r\n", res);
            return 0;//8
        }
        printf("[mpu_dmp_init] Self-test successful\r\n");
        
        printf("[mpu_dmp_init] Enabling DMP state...\r\n");
        res=mpu_set_dmp_state(1);	//Enable DMP
        if(res) {
            printf("[mpu_dmp_init] DMP state enable failed, error code: %d\r\n", res);
            return 9;
        }
        printf("[mpu_dmp_init] DMP state enabled successfully\r\n");
        
        printf("[mpu_dmp_init] DMP initialization completed successfully!\r\n");
    } else {
        printf("[mpu_dmp_init] MPU6050 basic initialization failed\r\n");
        return 10;
    }
    return 0;
}
